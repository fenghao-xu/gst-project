...
public class ExportCustomDataUtil {
    public static void exportData(Integer type, CustomStyleCraftCourse data, HttpServletResponse response,
                                  List<CraftVO> craftVOS,
                                  String weftElasticGrade, String warpElasticGrade) throws Exception {
...
//            String basePath = "D:\\Users\\Administrator\\Documents\\WXWork\\1688854099102101\\Cache\\File\\2020-09\\";
            File file = new File(basePath + "templateCustom.xlsx");
            ...
            String fileName = "";
            String excelName = "";
            String productionTicketNo = "";
            String orderCode = data.getOrderNo() + "-" + data.getOrderLineId();
            if (StringUtils.isNotBlank(productionTicketNo)) {
                fileName = path + File.separator + productionTicketNo + typeName + "导出.xlsx";
                excelName = productionTicketNo + typeName + "导出.xlsx";
            } else {
                fileName = path + File.separator + orderCode + typeName + "导出.xlsx";
                excelName = orderCode + typeName + "导出.xlsx";
            }
            ...
            if (resultfile.exists()) {
                ...
                writer.getOrCreateCell(0, 0).setCellValue(data.getFactoryName());
                writer.getOrCreateCell(9, 0).setCellValue(sdf.format(new Date()));

                writer.getOrCreateCell(1, 2).setCellValue(orderCode);
                if (StringUtils.isNotBlank(data.getProductionCategory())) {
                    writer.getOrCreateCell(5, 2).setCellValue(data.getProductionCategory());
                }
                String subbrandName = data.getBrand()!=null ? data.getBrand() : "";
                writer.getOrCreateCell(10, 2).setCellValue(subbrandName);

                writer.getOrCreateCell(1, 3).setCellValue(data.getStyleCode());
                String version = "版本号："+(data.getReleaseVersion()==null?"":data.getReleaseVersion());
                writer.getOrCreateCell(3, 3).setCellValue(version);
                String desc = "款式描述：" + (data.getStyleDescript()==null?"":data.getStyleDescript());
                writer.getOrCreateCell(4, 3).setCellValue(desc);
...
//                    writer.setStyle(style1, 3, rowNumber);
                ...
                //处理图片
                int pictureHeight = 7;
                if (StringUtils.isNotBlank(data.getStylePicture())) {
                    insertPictureToExcel(data.getStylePicture(), path, sheet, wb, 5, 4, 10, 4 + pictureHeight);
                }
                ...
                writer.merge(rowNumber, rowNumber, 6, 7, "机器名称", false);
                ...
                if (null != craftVOS && craftVOS.size() > 0) {
                    for (CraftVO vo : craftVOS) {
                        ...
                        writer.setStyle(leftStyle, 4, rowNumber);
                        writer.setStyle(leftStyle, 5, rowNumber);
                        writer.merge(rowNumber, rowNumber, 6, 7, vo.getMachineName(), false);
                ...
                writer.getOrCreateCell(1, rowNumber).setCellValue(data.getCreateUser());
                ...
                String createTimeStr = data.getCreateTime() != null ? sdf.format(data.getCreateTime()) : "";
                ...
                writer.getOrCreateCell(6, rowNumber).setCellValue(data.getReleaseUser());
                ...
                String releaseTimeStr = data.getReleaseTime() != null ? sdf.format(data.getReleaseTime()) : "";
                ...
                String sheetName ="定制订单"+typeName;
    ...
    public static void exportFinanceData(Integer type, CustomStyleCraftCourse data, HttpServletResponse response,
                                         List<CraftVO> craftVOS, String weftElasticGrade, String warpElasticGrade) throws Exception {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        XSSFWorkbook wb = null;
        ExcelWriter writer = null;
        ServletOutputStream fos = null;
        ...
        try {
            ...
            if (StringUtils.isNotBlank(productionTicketNo)) {
                ...
                fileName = path + File.separator + orderCode + typeName + ".xlsx";
                excelName = orderCode + typeName + ".xlsx";
            }
            resultfile = new File(fileName);
            try {
                FileUtils.copyFile(file, resultfile);
