 ...
 * @Description:PI品类主数据接收
 * @Date: Created in 10:32 2020/3/10
 */
@CrossOrigin(origins = "*", maxAge = 3600)
@Controller
@RequestMapping(value = "/categoryMasterData")
public class CategoryMasterDataSynController {
    @Resource
    ...
    @ResponseBody
    @RequestMapping("/tgetCategoryData")
    public ResultObject getMainFabricData(HttpServletRequest request) throws Exception {
        ...
        // 1、解析data数据
        DataParent<CategoryMasterData> parent = (DataParent<CategoryMasterData>) JSON.parseObject(data, new TypeReference<DataParent<CategoryMasterData>>() {});
        ...
        // 3、获取主数据
        PiDataUtil<CategoryMasterData> util = new PiDataUtil<CategoryMasterData>();
        List<DataChild<CategoryMasterData>> children = util.getChildData(parent, CategoryMasterData.class);
        ...
        List<Dictionary> dictionaryaList = IdictionaryService.getAll();
        Map<String,Object> dictionaryMap = new HashMap<>();
        for(Dictionary dictionary : dictionaryaList){
        ...
        for (DataChild<CategoryMasterData> child : children) {
            CategoryMasterData categoryMasterData = child.getItem();
                try {
                    Dictionary dictionary = new Dictionary();
                    dictionary.setDicValue(categoryMasterData.getCategoryCode());
                    if(categoryMasterData.getCategoryType()!=null||categoryMasterData.getCategoryType()!=""){
                    if("大类".equals(categoryMasterData.getCategoryType())){
                        dictionary.setDictionaryTypeCode("ClothesCategory");
                    }else if("中类".equals(categoryMasterData.getCategoryType())){
                        dictionary.setDictionaryTypeCode("ClothesCategory2");
                    }else{
                        dictionary.setDictionaryTypeCode("ClothesCategory3");
                    }
                    }
                    dictionary.setValueDesc(categoryMasterData.getCategoryName());
                    dictionary.setParentCode(categoryMasterData.getTopCategoryCode());
                    if(dictionaryMap.containsKey(dictionary.getDicValue()+"_"+dictionary.getDictionaryTypeCode())){
                        ...
                        IdictionaryService.addDictionary(dictionary);
                    }
